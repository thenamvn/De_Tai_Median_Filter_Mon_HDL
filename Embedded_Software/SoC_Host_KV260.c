...
int main() {
   ...
   int height, width;
   uint8_t **input = read_pgm("Fig3.pgm", &height, &width);
   uint8_t **output = allocate_image(height, width);

   write_uint8_matrix_to_hex_txt("Fig3.txt", input, width, height);
   ...
   *(CGRA_info.pio_32_mmap + WIDTH_PIO_BASE)  = width; // Bắt đầu xử lý trên FPGA
   *(CGRA_info.pio_32_mmap + HEIGHT_PIO_BASE) = height; // Bắt đầu xử lý trên FPGA
